...
In Serve WebUI, the `+` button opens separate **Upload image** and **Upload file** choices. Image selection, clipboard paste, and drag/drop accept PNG, JPEG, GIF, and WebP images up to 10 MiB each; multiple images and files can be added to one draft. Images appear as removable thumbnails and other files as removable cards without exposing their storage paths in the composer. Removing a draft card only removes that message's reference. Uploads are streamed, show progress, can be cancelled, and keep the current text plus already completed cards after a failure.

Image understanding uses the selected model's existing vision support. A text-only model does not switch automatically: the composer keeps a visible warning and Baize may still try an existing vision sub-agent or image tool. Attachment-only messages are supported. Attachments can be sent in Standard and Plan modes, but are intentionally unavailable for Goal drafts and guidance sent while a turn is running. Stored attachments use `0600` permissions under the workspace `.reasonix/attachments/` until the user deletes one or clears all from Storage. Deletion can invalidate historical references and rotates the current session temporary directory so Inline analysis caches are invalidated; reports and exports are retained. Interrupted, cancelled, over-limit, and over-quota uploads leave no partial file.
