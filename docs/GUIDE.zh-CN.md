...
Serve WebUI 的 `+` 按钮会向上展开“上传图片”和“上传文件”。图片选择、剪贴板粘贴和拖放支持 PNG、JPEG、GIF、WebP，单张上限为 10 MiB；一条草稿可同时添加多张图片和多个文件。图片显示为可预览、可移除的缩略图，其他文件显示为文件卡片，输入框不会暴露存储路径。移除草稿卡片只会解除当前消息的引用。上传使用流式协议，支持进度和取消；失败时会保留输入文字以及已上传完成的卡片。

图片理解复用当前模型已有的 vision 能力，不会自动切换模型。使用纯文本模型时，输入区会常驻提示；Baize 仍可尝试现有视觉子代理或图片工具。可以只发送附件。附件支持常规模式和 Plan 模式，但 Goal 草稿与任务运行期间的跟进消息暂不支持附件。附件以 `0600` 权限保留在工作区 `.reasonix/attachments/`，直到用户在 Storage 页面单项删除或全部清理。删除可能令历史消息中的附件引用失效，并同步轮换当前会话临时目录，使 Inline 数据分析缓存失效；报告和导出文件不会随附件删除。上传中断、取消、超限或配额不足不会保留半文件。
...
Starlark 不是 Python：应使用 `== None` 和 `sorted()`；不支持 `is`、`**`、内置
`round`，字符串 `%` 也不支持 Python 的宽度和精度格式。`analyze_data` 应只承担简单
JSON 计算。确实需要标准 Python 数学或格式化语义时，Bash 可传
`execution_scope: "scratch"`：工作区只读，会话私有
临时目录是唯一写入根，网络、后台进程、额外写目录和沙箱逃逸均被禁用；Linux 下该
目录映射为 `/tmp`。Scratch 仅在宿主能证明 OS 沙箱可用时执行，沙箱不可用会直接
拒绝而不会退化为无隔离执行，此时不得回退到普通 Bash。前台 Bash 还可通过可选的
