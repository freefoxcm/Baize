...
| OpenCode Go or proxy `glm-5.3-flash` | `opencode.ai/zen/go/v1` or any OpenAI-compatible proxy | `reasoning_effort` | `auto`, `low`, `high`, `max` | Recognised by exact model ID. `auto` omits the field; an explicit `reasoning_protocol = "none"` still disables the control. |
