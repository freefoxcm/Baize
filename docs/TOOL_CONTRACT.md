...
| --- | --- | --- |
| `analyze_data` | true | Run simple deterministic, side-effect-free Starlark calculations over JSON for grouping, aggregation, intersections, rankings, and straightforward trends without creating temporary files or accessing files, environment variables, time, randomness, network, modules, or subprocesses. Starlark is not Python: use `== None` and `sorted()`; `is`, `**`, built-in `round`, and Python width/precision `%` formatting are unavailable. Use a fixed Python script only through `bash` with `execution_scope:"scratch"` when standard Python semantics are required; never fall back to normal Bash. |
| `bash` | false | Execute a command in the shell and return combined stdout/stderr. Use for builds, tests, git, package managers, etc. Pass bounded structured input through stdin instead of shell interpolation or workspace request files; stdin is foreground-only and capped at 2 MiB. To search/read/list/edit/move files, prefer the dedicated tools (grep, read_file, ls, glob, edit_file, move_file) over shell grep/cat/ls/find/sed/mv/Move-Item - they behave identically on every OS. For symbol search or architecture questions, prefer LSP/read tools and targeted grep before shell commands. |
...
| `complete_step` | true | Record the evidence-backed completion of one approved-plan step. Evidence may cite a successful verification, read/query tool, host-confined computation, review, diff/files, or manual check; `tool` and `computation` evidence must match a successful output-producing receipt from the current step. |
