...
func TestStandardTodoContinuationCompletesInsideSameRun(t *testing.T) {
	a, prov := standardTodoTestAgent(t, [][]provider.Chunk{
		{toolCallChunk("todo-1", "todo_write", `{"todos":[{"content":"Edit code","status":"in_progress","step_id":"edit_01"}]}`), {Type: provider.ChunkDone}},
		{{Type: provider.ChunkText, Text: "I will edit it next."}, {Type: provider.ChunkDone}},
		{
			toolCallChunk("write-1", "write_file", `{"path":"main.go"}`),
			toolCallChunk("signoff-1", "complete_step", `{"step_id":"edit_01","result":"implementation complete","evidence":[{"kind":"manual","summary":"main.go updated and inspected"}]}`),
	...
	if prov.call != 4 {
		t.Fatalf("provider calls = %d, want 4; extra request tail = %q; last todo result = %q; last complete_step result = %q", prov.call, lastUser(prov.requests[len(prov.requests)-1]), lastToolResult(a.Session(), "todo_write"), lastToolResult(a.Session(), "complete_step"))
	}
