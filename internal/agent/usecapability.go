...
func (*UseCapabilityTool) Schema() json.RawMessage {
	...
	return json.RawMessage(`{
		...
		"properties":{
			...
			"arguments":{"oneOf":[{"type":"object"},{"type":"string"}],"description":"Raw tool arguments for action=call. Skill capabilities also accept a freeform string."},
		...
		if tl, ok := t.registry.Get(toolName); ok {
			...
			if len(payload) == 0 || string(payload) == "null" {
				...
				if json.Unmarshal(payload, &freeform) == nil {
					payload, _ = json.Marshal(map[string]any{"name": skillName, "arguments": freeform})
				} else {
