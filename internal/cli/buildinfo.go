...
// `baize version` / `baize --version`. Release builds may only fill
...
//   - `baize --version` / `-v` stay single-line: "baize <version>"
//   - `baize version --verbose` prints structured fields
//   - `baize version --json` prints a JSON object
...
// versionCommand handles `baize version [flags]`. When allowFlags is false
...
func versionUsage(w *os.File) {
  ...
  baize version
  baize version --verbose
  baize version --json
  baize --version
  baize -v

--version / -v always print a single line (baize <version>).
version --verbose prints build metadata; version --json prints the same as JSON.`)
}

func (b BuildInfo) singleLine() string {
	return "baize " + strings.TrimSpace(b.withDefaults().Version)
}
