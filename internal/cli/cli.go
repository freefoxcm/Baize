...
func RunWithBuildInfo(args []string, info BuildInfo) int {
	...
	// -p/--print is one-shot print mode. baize has no interactive -p, so a
	// print flag anywhere in a leading flag run (no explicit subcommand) routes
	// the whole set to `run --print` — `baize --model X -p "task"` works, not
	// only `baize -p ...`.
	...
	case "init":
		...
		// for config), so `baize init` isn't a dead end.
		configureCLIThemeFromConfig()
		return initHint()
	case "acp":
		configureCLIThemeFromConfig()
...
func runAgent(args []string, version string) int {
	...
	*permissionMode = resolvedPermissionMode
	...
	// `baize run` is headless: there is no key loop to answer approval or ask
...
func chatREPL(args []string, version string) int {
	...
	fs := pflag.NewFlagSet("baize", pflag.ContinueOnError)
	fs.SetInterspersed(true)
	model := fs.String("model", "", "provider name (default: config default_model)")
	maxSteps := fs.Int("max-steps", 0, "one-off max tool-call rounds (0 = automatic)")
	cont := registerContinueFlag(fs)
...
// resolveSetupTargets picks where `baize setup` writes. Keys always go to the
...
// setupConfig runs the configuration wizard (the `baize setup` command),
...
// initHint handles `baize init`. Unlike a config scaffold, project memory is
...
// (and to `baize setup` for config) so the verb isn't a dead end.
func initHint() int {
...
func interactiveSetup(configPath, envPath string) int {
	...
	fmt.Print(boxed([]string{
		accent("◆") + " " + fmt.Sprintf(i18n.M.WelcomeTitleFmt, bold("baize")),
		"",
...
// duplicate provider entries on subsequent re-runs of `baize setup`.
...
// are appended — so re-running `baize setup` with a corrected key replaces the
