...
func buildReviewSubagentRegistry(reviewSk skill.Skill, cfg *config.Config, root string) *tool.Registry {
	...
	// [sandbox] config, so `baize review` previously read forbid_read
	...
	if reviewSk.ReadOnly {
		...
		// permission-classified read-only policy) so `baize review` is not a
		// writable backdoor.
