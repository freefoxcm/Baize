...
func TestTaskUnknownSubcommandsNameValidOnes(t *testing.T) {
	...
	for _, tc := range []struct {
	...
	}{
		{
			...
			wantUsage: "usage: baize task <list|show|monitor|status|events|stop|cancel|requeue|open-session|tmux> [flags]",
		},
		{
			name:      "task monitor",
			run:       func() int { return taskMonitorCommand(store, []string{"bogus"}) },
			wantUsage: "usage: baize task monitor <list|status|events|stop|cancel|requeue|open-session> [flags]",
		},
		{
			name:      "task tmux",
			run:       func() int { return taskTmuxCmd(store, []string{"bogus"}) },
			wantUsage: "usage: baize task tmux <attach|status|open|detach>",
		},
	} {
		t.Run(tc.name, func(t *testing.T) {
