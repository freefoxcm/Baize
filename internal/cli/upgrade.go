...
var (
	...
	requiredCLIAssets   = [...]string{
		"baize-darwin-amd64.tar.gz",
		"baize-darwin-arm64.tar.gz",
		"baize-linux-amd64.tar.gz",
		"baize-linux-arm64.tar.gz",
		"baize-windows-amd64.zip",
		"baize-windows-arm64.zip",
		"SHA256SUMS",
	}
)
...
// upgradeCommand handles `baize upgrade` (and `baize update`).
func upgradeCommand(args []string, version string) int {
	...
	base := fmt.Sprintf("baize-%s-%s", runtime.GOOS, runtime.GOARCH)
...
func cliPlatformAssetName(goos, goarch string) string {
	...
	return fmt.Sprintf("baize-%s-%s%s", goos, goarch, suffix)
}
...
func fetchLatestRelease(c *http.Client, channel cliReleaseChannel) (*ghRelease, error) {
	...
	req.Header.Set("User-Agent", "baize-cli")
...
// extractBinary pulls the "baize" binary from a .tar.gz or .zip archive.
...
// rename it aside to .baize.old first, then place the new binary.
