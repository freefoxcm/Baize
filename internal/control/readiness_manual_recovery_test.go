...
func TestStandardStartReplyContinuesCurrentTodoInsideVisibleTurn(t *testing.T) {
	c, prov := manualReadinessController(t, [][]provider.Chunk{
		{toolCallChunk("todo-1", "todo_write", `{"todos":[{"content":"重写第 4 节","status":"in_progress","step_id":"rewrite_04"}]}`), {Type: provider.ChunkDone}},
		textTurn("让我先列出待办，接下来会重写。"),
		{
			toolCallChunk("write-1", "write_file", `{"path":"PRD.md"}`),
			toolCallChunk("signoff-1", "complete_step", `{"step_id":"rewrite_04","result":"section rewritten","evidence":[{"kind":"manual","summary":"PRD.md section 4 updated and inspected"}]}`),
