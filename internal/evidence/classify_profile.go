...
func ClassifyEffect(in EffectInput) EffectProfile {
	...
	if in.Scope == effectscope.Scratch {
		profile := readOnlyProfile(targetsFrom(in, nil), ReasonReadOnly)
		profile.Scope = effectscope.Scratch
		return profile
	}
...
func classifyBashEffect(args json.RawMessage, in EffectInput) EffectProfile {
	...
	if strings.EqualFold(stringField(fields, "execution_scope"), "scratch") {
		profile := readOnlyProfile(nil, ReasonReadOnly)
		profile.Scope = effectscope.Unknown
		return profile
	}
