...
func (analyzeData) Description() string {
	return "Run simple deterministic, side-effect-free Starlark calculations over JSON. Use MCP/read tools to obtain facts first, then pass their JSON here for grouping, aggregation, intersections, rankings, or straightforward trends without temporary files. The program receives `input` and must assign its JSON-compatible answer to global `result`; put loops inside a function and call it when assigning result. Starlark is not Python: use `== None` instead of `is`, use `sorted()` for ordering, and do not use `**` or built-in `round`; string `%` does not support Python width or precision formatting. It has no file, environment, clock, random, network, module-loading, or subprocess access. When ordinary Python math or formatting is genuinely required, use a fixed script through bash with `execution_scope=scratch`; never fall back to normal bash when scratch isolation is unavailable."
}

func (analyzeData) Schema() json.RawMessage {
	return json.RawMessage(`{"type":"object","properties":{"program":{"type":"string","maxLength":32768,"description":"Starlark source for simple JSON calculations. It receives global input and must assign a JSON-compatible value to global result. This is not Python: use == None, sorted(), and explicit arithmetic; is, **, built-in round, and Python width/precision % formatting are unavailable."},"input":{"description":"JSON data to analyze (maximum encoded size 2 MiB)."}},"required":["program","input"]}`)
}
